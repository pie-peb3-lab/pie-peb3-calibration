...
EVIDENCE_PATH = "peb3e-controlled-calibration-v2-evidence.json"
...
def request_json(url, *, method="GET", token=None, data=None):
    headers = {
        ...
        "User-Agent": "pie-peb3-controlled-calibration-v2",
    }
    if token:
...
def graphql(token, query, variables):
    ...
    if result.get("errors"):
        raise RuntimeError(f"GraphQL error: {json.dumps(result['errors'])[:500]}")
    return result["data"]
...
def main():
    evidence = {
        "contract_version": "PEB3E_CONTROLLED_NONPRODUCTION_PR_STATE_CALIBRATION_V2",
        "provider": "GITHUB",
        "target": {
    ...
    try:
        ...
        installation_permissions = installation.get("permissions") or {}
        expected_permissions = {"contents": "write", "metadata": "read", "pull_requests": "write"}
        if installation_permissions != expected_permissions:
            raise RuntimeError(f"installation permission mismatch: {installation_permissions}")

        token_response = request_json(
            f"{API}/app/installations/{installation['id']}/access_tokens",
            method="POST",
            token=app_jwt,
            data={"repositories": [REPO], "permissions": {"contents": "write", "pull_requests": "write"}},
        )
        ...
        if permissions != expected_permissions:
            raise RuntimeError(f"token permission mismatch: {permissions}")
